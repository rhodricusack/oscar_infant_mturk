...
var wsURL = "wss://wowza-cusacklab.org/webrtc/webrtc-session.json";
...
function wsConnect(url)
{
	...
	wsConnection.onopen = function()
	{
		...
		peerConnection.onconnectionstatechange = function(event) {
			if (cmt.webcam_events.length<=cmt.stimnumber)
				cmt.webcam_events[cmt.stimnumber]=[];
			cmt.webcam_events[cmt.stimnumber].push({'type':peerConnection.connectionState,'wallclock': Date.now()});
		  };
...
function startPublisher(streamName)
{
	wsURL = 'wss://wowza-cusacklab.org/webrtc/webrtc-session.json';
